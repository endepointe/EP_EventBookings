...
const findOrCreate = async (data) => {
	...
	try {
		...
		if (user === null || !user) {
			const newuserres = await db.one('insert into googleusers(id,name,avatar_url,provider) values($1,$2,$3,$4) returning id', [data.id, data.name, data.avatar_url, data.provider]);
